...
exports.handler = async (event) => {
  ...
  try {
    ...
    const idx = volunteers.findIndex(v => v.id === parseInt(body.id));
    if (idx === -1) throw new Error(`Volunteer ID ${body.id} not found`);
    volunteers[idx] = { ...volunteers[idx], ...body, id: volunteers[idx].id };
    await githubWriteJSON(githubToken, 'data/volunteers.json', sha, volunteers, `Edit volunteer: ${body.name}`);
