...
exports.handler = async (event) => {
  ...
  try {
    ...
    const idx = pending.findIndex(v => v.id === parseInt(body.id));
    if (idx === -1) throw new Error(`Pending volunteer ID ${body.id} not found`);
    pending[idx] = { ...pending[idx], ...body, id: pending[idx].id };
    await githubWriteJSON(githubToken, 'data/pending.json', sha, pending, `Edit pending volunteer: ${body.name}`);
