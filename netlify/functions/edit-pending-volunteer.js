...
exports.handler = async (event) => {
  ...
  try {
    ...
    await updateGitHubFile({
      token: token,
      path: 'js/shared.js',
      commitMessage: `Edit pending volunteer ${data.name} (Pending ID: ${id})`,
      updateFn: (content) => {
        ...
        for (let i = 0; i < lines.length; i++) {
          ...
          if (insidePending && idRegex.test(lines[i])) {
        ...
        if (!updated) {
          throw new Error(`Pending volunteer with ID ${id} not found in PENDING_VOLUNTEERS_DATA inside shared.js`);
        }
    ...
    await updateGitHubFile({
      ...
      commitMessage: `Sync Pending CSV for volunteer edit ${data.name} (Pending ID: ${id})`,
      updateFn: () => {
