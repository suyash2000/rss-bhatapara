...
exports.handler = async (event) => {
  ...
  try {
    ...
    const idx = volunteers.findIndex(v => v.id === parseInt(id));
    if (idx === -1) throw new Error(`Volunteer ID ${id} not found`);
    const [removed] = volunteers.splice(idx, 1);
    await githubWriteJSON(githubToken, 'data/volunteers.json', sha, volunteers, `Delete volunteer: ${removed.name}`);
